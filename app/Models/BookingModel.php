...
class BookingModel extends Model
{
    ...
    public function AddBooking($data){
        ...
        $result = $this->db->table('booking')->insert($data);

        $insertId = $this->db->insertID();
        return ["result"=>$result,"insert_id" => $insertId];
    }
    ...
    public function ViewBookings(){
        ...
        foreach($result->getResult() as $record => $value ){
            $agent_details = (new AgentModel())->ViewSingleAgent($value->agent_id);
            if($agent_details){
                $value->agent_name = $agent_details[0]->full_name;
            }else{
                $value->agent_name = "No Details";
            }
        }
